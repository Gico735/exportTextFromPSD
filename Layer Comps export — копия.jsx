...
/*
@@@BUILDINFO@@@ Layer Comps export.jsx 1.0.0.24
*/
...
<javascriptresource>
<name>Layer Comps export...</name>
<category>scriptexport</category>
<menu>export</menu>

    <enableinfo>PSHOP_DocHasLayerComps</enableinfo>
<eventid>cf34b502-2013-4d07-8431-1dfd634ee0co</eventid>
<terminology><![CDATA[<< /Version 1
                         /Events <<
                          /cf34b502-2013-4d07-8431-1dfd634ee0co [($$$/JavaScripts/LayerCompsToFiles/Action=Layer Comps export) /noDirectParam <<
                          >>]
                         >>
                      >> ]]>
</terminology>
...
var strTitle = localize("$$$/JavaScripts/LayerCompsToFiles/Title=Layer Comps export");
...
var strMessage = localize("$$$/JavaScripts/LayerCompsToFiles/Message=Layer Comps export action settings");
...
function main() {
	...
	try {
		var d = app.getCustomOptions("d69fc733-75b4-4d5c-ae8a-c6d6f9a8aa329");
	...
	try {
		...
		if (compsCount < 1) {
			...
			var docPath = app.activeDocument.path.toString().replace(/(_process|psd)(?=(.$|$))/gi, 'PNG');
			if (exportInfo.destinationPNG) {
				exportInfo.destination = docPath;
			}
			...
			var myHistory = docRef.historyStates.length - 1;
			...
			for (compsIndex = 0; compsIndex < compsCount; compsIndex++) {
				...
				compRef.apply();
				var fileNameBody = docName.replace('.psd', '');
				if (exportInfo.fileNamePrefix.length) {
					fileNameBody += "_" + exportInfo.fileNamePrefix;
				}
				// fileNameBody += "_c" + zeroSuppress(compsIndex, 2);
				fileNameBody = fileNameBody.replace(/[:\/\\*\?\"\<\>\|\\\r\\\n]/g, "_");  // '/\:*?"<>|\r\n' -> '_'
				if (fileNameBody.length > 120) fileNameBody = fileNameBody.substring(0, 120);
				saveFile(docRef, fileNameBody, exportInfo);
			}
			docRef.activeHistoryState = docRef.historyStates[myHistory];
			app.purge(PurgeTarget.HISTORYCACHES);
			docRef.layerComps[0].apply();
			var d = objectToDescriptor(exportInfo, strMessage, preProcessExportInfo);
			app.putCustomOptions("d69fc733-75b4-4d5c-ae8a-c6d6f9a8aa329", d);
...
function settingDialog(exportInfo) {
	...
	dlgMain.destinationPNG = dlgMain.grpTopLeft.add("checkbox", undefined, 'PNG');
	dlgMain.destinationPNG.value = exportInfo.destinationPNG;
	...
	dlgMain.btnRun.onClick = function () {
		...
		var destinationPNG = dlgMain.destinationPNG.value;
		if (destination.length == 0 && !destinationPNG) {
			alert(strAlertSpecifyDestination);
			return;
		}
		if (destinationPNG) {
			pngPath = app.activeDocument.path.toString().replace(/(_process|psd)(?=(.$|$))/gi, 'PNG');
			folderExists(pngPath);
			var testFolder = new Folder(pngPath);
		} else {
	...
	if (exportInfo.destinationPNG) {
		exportInfo.destination = pngPath;
	} else {
...
function initExportInfo(exportInfo) {
	...
	exportInfo.destinationPNG = true;
